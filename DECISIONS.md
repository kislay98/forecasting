...
| M6 | Diagnostics, report, CLI `run` and `report` | Next |
...
- Entry point: `forecasting.evaluation.scoring.score_run(run_dir)` returns every table the report needs. `Scores.point` has one row per (series, window, model, h), with `test_h` marking the horizons where tests count and `reference` naming the baseline.
- Print n and n_eff next to every statistic (spec: Horizons). They are columns in point, intervals and interval_buckets.
- RQ1 verdict: use `dm_p_better_holm` at the test horizons plus `rel_mae < 1`, and MCS membership. Suspiciously large gains (over 30% vs ETS) call for a leakage audit (Phase 1 exit table).
- RQ2: `h_star` is per model. The report has to say which model's h* is the answer (the best model on dev, or the combination) without choosing it from test results.
- RQ3: use `interval_buckets` (coverage, band, Kupiec per bucket and level). With 30 monthly test origins the medium and long bands are [0, 1], so they say nothing. The report should say this rather than print "in band".
- Skill CIs at h close to n are unreliable. With a moving (not circular) block bootstrap and block length h near n, the middle origins are over-sampled and the percentile CI can miss the point estimate (example run: ETS at h = 24 with n = 30 has skill 0.323 and CI [0.327, 0.429]). Flag horizons where block > n / 3 in the report. A circular block bootstrap would fix it, but would change the spec's method.
- For return series the store's y_true at h is a one-period return, so errors at different origins barely overlap, and n / h (and DM lags h - 1) is conservative. The spec rule is kept; mention it in the report. Cumulative h-day returns (from level_true / level_pred, M2-3) are not scored yet.
- On the synthetic index_like series AR(p) always chose p = 0 and forecasts exactly like mean_return, so the two tie in the MCS and in DM. Expect the same on Nifty. The report should say so rather than list them as two findings.
- Residual diagnostics (Ljung-Box, ARCH-LM on one-step residuals of each test origin) and the rolling vs expanding gap (RQ5) are not in M5. The store has no residuals, so RQ4 needs either a stored residual summary or a refit-free route. Decide this before writing the report.
